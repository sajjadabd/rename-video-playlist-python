...
for count, filename in enumerate(os.listdir(__dirname)) :
    ...
    if( file_extension[1] == __EXTENSION ) :
        fileNumber = int(file_extension[0].split('-')[0])
...
for count, filename in enumerate(os.listdir(__dirname)) :
    ...
    if( file_extension[1] == __EXTENSION ) :
        ...
        precedence = '0'*  ( len(str(__MAXIMUM_NUMBER)) - int(len(file_extension[0].split('-')[0])) )
        #print(precedence + file_extension[0] + file_extension[1]);
        source = filename
        destination = precedence + file_extension[0] + file_extension[1]
